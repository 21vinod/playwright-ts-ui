...
test('Login to Dice.com and Search and Apply', async ({ browser }) => {
    ...
    await page.fill('input[name="email"]', '21vinod12@gmail.com');
    await page.click('button[type="submit"]');
    await page.fill('input[name="password"]', 'Vinod55@');
    await page.waitForTimeout(2000);
    await page.click('button[type="submit"]');

    await page.waitForSelector('input[placeholder="Job title, skill, company, keyword"]', { state: 'visible' });
    await page.waitForTimeout(2000);
    await page.getByRole('combobox', { name: 'Job title, skill, company,' }).fill('Quality Assurance Engineer');//"Quality Assurance Engineer" OR "QA Engineer" OR "QA Analyst" OR "Software Tester" OR "Test Engineer" OR "Automation Engineer" OR "SDET" OR "Software Development Engineer in Test"
...
async function singlePageApply(easyApplyButtons) {
    for (const button of easyApplyButtons) {
        ...
        //upload the resume
        await newPage.setInputFiles('input[type="file"]', 'X:/Job applications/QA profile/Own profile/9 years/Sr QA Cover.docx');
