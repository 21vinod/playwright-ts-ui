...
test('Login to Dice.com and Search and Apply', async ({ browser }) => {
    ...
    await page.getByRole('combobox', { name: 'Job title, skill, company,' }).fill('Quality Assurance Engineer');
...
async function singlePageApply(easyApplyButtons) {
    for (const button of easyApplyButtons) {
        ...
        try {//wait for the easy apply button to be visible
            ...
            // Optionally, you can close the new page if an error occurs
            await newPage.close();
        }
    }
}
